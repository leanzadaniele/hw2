...
function getUserLogged(){
    return fetch("getUserLogged").then(onResponse).then((data) => {
        return data;
    });
...
function setLike(event){
    ...
    console.log("Post_ID");
    console.log(post_id);
    // document.querySelector("#user_id").value old code
    window.currentTarget = event.currentTarget;
    let user_id = 0;
    getUserLogged().then((id) => {
        if(id > 0){
            user_id = id;
            console.log(user_id);
            ...
            window.currentTarget.classList.toggle("liked");
            const likes = window.currentTarget.parentNode.querySelector("p");
            if(window.currentTarget.classList.contains("liked")){
...
function checkNComment(event){
    ...
    if(event.key === "Enter" && event.currentTarget.value.length !== 0){
        ...
        const correctComment = encodeURIComponent(comment);
            ...
            window.id_post = event.currentTarget.parentNode.parentNode.dataset.postId;
            /* const user_id = document.querySelector("input[type='hidden']").value; */
            getUserLogged().then((id) => {
                const user_id = id;
                const id_post = window.id_post;
                const url = "comment/" + correctComment + "/" + user_id + "/" +id_post;
                console.log(url);
...
function onJsonSetLiked(json){
    //console.log(json);
    for(let i = 0; i<json.length; i++){
        let likeBTN = document.querySelector(".like[data-post-id='" + CSS.escape(json[i].post_id) + "'] .likeBTN");
